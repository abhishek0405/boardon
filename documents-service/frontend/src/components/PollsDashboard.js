...
function PollsDashboard() {
  ...
  const handleChange = (event) => {
    ...
    console.log(pid);
    var url =
      "http://localhost:3005/hr/viewAllresults?poll=" + event.target.value;
    fetch(url, {
      credentials: "include",
      headers: { Authorization: `Bearer ${localStorage.getItem("authToken")}` },
    })
  ...
  return (
    <div
      className="PollsDashboard"
      style={{ position: "absolute", top: "15%", left: "20%" }}
    >
      {typeof backendData.allCharts === "undefined" ? (
        <p>Loading...</p>
      ) : (
        <>
          <h1>Poll: {pollName}</h1>
          <select value={selected} onChange={handleChange}>
            {backendData.currPolls.map((cp) => {
              return (
                <option key={cp.pollName} value={cp.pollId}>
                  {cp.pollName}
                </option>
              );
            })}
